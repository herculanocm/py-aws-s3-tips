...
def unzip_bytes_upload_s3_objects(s3_client: aws_client, bytes_zip: bytes, target_bucket: str, target_key_prefix: str, logger=None):
    log = get_logger(logger)
    for zipInfo in get_infolist_from_bytes_zip(bytes_zip, log):
        ...
        bytes_obj = get_file_from_bytes_zip(bytes_zip, file_name, log)

        upload_fileobj(s3_client, bytes_obj, target_bucket, final_key, file_name, file_size, log)
